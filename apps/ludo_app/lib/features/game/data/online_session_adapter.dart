...
class OnlineSessionAdapter {
  ...
  Future<void> apply(
    ...
    for (final plan in plans) {
      ...
      switch (plan.kind) {
        case _TokenMoveKind.stay ||
          ...
          final destination = plan.to ?? 0;
          if (destination < 0) {
            // Still in base ('stay' with progress -1). Must never fall into
            // the path lookup below: clamping -1 to 0 would teleport every
            // base token onto its team's start cell.
            token.positionId = plan.baseSpot ?? token.positionId;
            token.state = engine.TokenState.inBase;
            break;
          }
          final progress = destination.clamp(0, _finishProgress).toInt();
    ...
    //    Capture thunks start only after the mover lands; animatePathBackward
    //    mutates positionId down to the start cell, so the base spot is
    //    restored afterwards (what offline moveTokenToBase does) before the
    //    final hop onto the base plate.
    final animations = <Future<void>>[];
    final moveFutures = <Future<void>>[];
    final captureStarters = <Future<void> Function()>[];
      ...
      if (detailed) {
        switch (plan.kind) {
          ...
          case _TokenMoveKind.captured when plan.baseSpot != null:
            captureStarters.add(() async {
              await component.animatePathBackward(plan.path, plan.from, 0);
              plan.token.positionId = plan.baseSpot!;
              plan.token.state = engine.TokenState.inBase;
